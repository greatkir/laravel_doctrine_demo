    ...
    $.getJSON('/index.php/api/job/' + curr_id, function (data) {
        if (data.statusCode === 200) {
            if (!(Object.keys(data.job).length === 0)) {
                ...
                $('.article-container').append('<p>' + date.getDate() + '/\n\
' + (date.getMonth() + 1) + '/' + date.getFullYear() + '  ' + data.job.location + ' \n\
  ' + data.job.brand + '</p>');
                $('.article-container').append('<p class="text-left" style="word-wrap:break-word;">\n\
' + data.job.description + '</p>');
